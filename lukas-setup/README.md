...
deinem Wiki-Partner, direkt aus Claude Code. Dein Rechner hält einen **lokalen Spiegel** der
Social-Branch; Änderungen synchronisieren sich per P2P (libp2p CRDT) über den
Relay auf dem Hetzner-Server des Partners mit dem Pi. Kein Cloud-Konto, kein Token, keine
Domain — nur libp2p (verschlüsselt via Noise + PeerIDs).

Nur `pages/social/` wird synchronisiert — die private Wiki des Partners bleibt bei ihm.
...
Der Node verbindet zum Relay, dialt den Partner-Pi und spiegelt `pages/social/`:
...
und zieht die vorhandenen Seiten vom Partner-Pi. Danach erscheinen sie unter
...
also automatisch beim Partner (und umgekehrt). `contributors` sammelt beide Autoren.
...
- Bei „Verbindung steht, aber nichts synct": prüfen, dass der Sync-Node läuft und
  die `WIKI_PEERS`-Adresse exakt stimmt (das ist die Partner-Pi-Circuit-Adresse oben).

## Referenz
- Relay-PeerID (Hetzner): `12D3KooWBsYoFG7J1xq6dEQTbZyN1qULncYXRirYgSEb12fYMaKL`
- Partner-Pi-Node-PeerID: `12D3KooWD9aujyjc34rK8rGm9NKi7tqar8UWNpZTAkyemR22E26U`
